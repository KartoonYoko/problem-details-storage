...
func configure(ctx context.Context) (*http_server.Controller, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("storage initilisation error: %w", err)
	}

	usecase := buckets.New(s)

	controller := http_server.New(
		http_server.Config{BaseAddress: ":8080"},
		usecase)
